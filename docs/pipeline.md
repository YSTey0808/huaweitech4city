...
Real-time system that flags cyberbullying/grooming/scam patterns in code-mixed conversations. Each message becomes a node in a per-conversation graph; a GraphSAGE GNN message-passes over that graph and produces a single conversation-level binary prediction (harmful/safe) plus a per-message contribution score. Both are signals, not a verdict — an LLM stage reads every message's actual text alongside those scores and makes the final call, since the GNN can be confidently wrong (most notably on very short conversations — see "LLM reasoning stage" below).
...
[`gnn/llm_stage.py`](../pipeline/gnn/llm_stage.py)'s `run_llm_reasoning(conversation_id, messages, conversation_score)` calls `LLM_MODEL` (`config.py` — currently Sonnet, chosen for judgment quality on this stage's actual job of reading nuance in message text, over the smaller/faster model this was originally tuned for) with the conversation ID, the conversation-level score, and **every message in the scoring window** (not a GNN-filtered subset) with each one's per-message contribution score. The prompt frames both scores explicitly as signals from a small classifier that can be wrong — not ground truth — and instructs the LLM to read the actual text and form its own judgment, overriding the classifier's label when the evidence doesn't support it. This matters concretely for single-message conversations: with only one message, `forward_full`'s pooling is a no-op (mean of one vector is itself), so that message's per-message score and the conversation score are mathematically identical — a bad GNN guess there is unfixable downstream unless something can override it, which is exactly what this stage now does. Output is a fixed JSON shape: `conversation_label` (one of `CONV_LABELS`, the LLM's own choice), `conversation_confidence`, `severity`, `top_evidence_messages` (up to `TOP_K_EVIDENCE`, whichever messages the LLM itself judges actually justify the verdict, with free-text LLM-authored tags), and a short `gentle_alert_text` for direct display to the user — this is what `backend/app/services/scoring_service.py` translates into `conversation_scores`/`message_scores` rows.

Two optional prompt inputs carry the human-feedback loop, both passed straight through by `score_conversation` and staying pure prompt text (the pipeline holds no Supabase/storage knowledge — the backend fetches them):

- **`user_report={message_id, reason, claim}`** — a member's report (`claim='harmful'`, "you missed this") or dispute (`claim='safe'`, "you flagged this wrongly"), added to the prompt as a strong human signal to weigh, not rubber-stamp. On a claim the LLM ends up disagreeing with, the prompt asks it to address the reporter's stated reason in `gentle_alert_text` (which the backend stores as the report's outcome explanation).
- **`confirmed_examples=[{text, reason, label}]`** — the "watchlist": past reports the LLM previously confirmed, from *any* conversation, injected as reference patterns (`harmful` ones as "known harmful patterns", `safe` ones as "known false-positive patterns"). This is what generalizes a single confirmed correction across all conversations before a retrain — see [backend.md](backend.md#watchlist--cross-conversation-generalization).
